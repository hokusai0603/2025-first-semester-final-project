...
class TradingConfig:
    ...
    調整建議：
    1. 模型更強大但訓練慢：增大 hidden_dims, embedding_dim, epochs
    2. 防止過擬合：增加 dropout, 減少 embedding_dim
    3. 更保守交易：提高 buy_threshold, 降低 sell_threshold
    4. 更激進交易：降低 buy_threshold, 提高 sell_threshold
    """
    
    # 模型架構參數
    EMBEDDING_DIM = 20  # 16->20 提升表徵能力
    HIDDEN_DIMS = (128, 64, 32)  # 增加層數和寬度
    DROPOUT = 0.15  # 0.1->0.15 防止過擬合
    
    # 訓練參數
    LEARNING_RATE = 1e-3
    EPOCHS = 50  # 40->50 更充分訓練
    EPISODES_PER_EPOCH = 80  # 60->80 更多 episode
    K_SHOT = 5  # Few-shot 支持樣本數
    Q_QUERY = 12  # 10->12 查詢樣本數
    TEMPERATURE = 0.8  # 1.0->0.8 更銳利的決策邊界
    
    # 交易策略參數
    BUY_THRESHOLD = 0.005  # 0.004->0.005 更保守（0.5%）
    HOLD_THRESHOLD = 0.002  # 持有區間
    SELL_THRESHOLD = -0.003  # 新增明確賣出門檻
    
    # 特徵工程參數
    RSI_PERIOD = 14
    SMA_PERIOD = 20
    ATR_PERIOD = 14
    LOG_RETURN_PERIODS = [5, 10, 20, 30]  # 增加30日收益率
    
    # 回測參數
    INITIAL_CAPITAL = 1000000
    COMMISSION_RATE = 0.002  # 0.2% 單邊手續費
    
    # 決策解釋參數
    ENABLE_EXPLANATION = True  # 是否輸出決策解釋
    TOP_FEATURES_TO_SHOW = 5  # 顯示前N個重要特徵
...
class ProtoNet:
    ...
    可調整參數建議：
    - embedding_dim: 8-32 (越大越能捕捉複雜模式，但易過擬合)
    - hidden_dims: (32,16) 快速, (128,64) 強大但慢
    - dropout: 0.0-0.3 (防止過擬合)
    - lr: 1e-4 保守, 1e-3 標準, 1e-2 激進
    - epochs: 20-80 (更多輪數可能提升但訓練時間長)
    - temperature: 0.5-2.0 (影響決策邊界的平滑度)
    """
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        if self.prototypes is None or self.class_to_index is None:
            raise RuntimeError("ProtoNet 尚未完成訓練或原型未建立，請先呼叫 fit() 後再進行預測機率")
        Z = self._embed(X)
    ...
    def explain_prediction(self, X: np.ndarray, feature_names=None) -> dict:
        ...
        for i in range(len(X)):
            ...
            # 計算到各原型的距離
            distances_to_protos = {
                f"到類別{c}({['賣出/觀望', '持有', '買入'][c]})距離": float(dists[i, c])
                for c in range(self.n_classes)
            }
            
            # 特徵敏感度：對每個特徵微擾後觀察距離變化
            feature_importance = {}
            epsilon = 0.01
            ...
            explanation = {
                ...
                'prediction_label': ['賣出/觀望', '持有', '買入'][int(predictions[i])],
                'probabilities': {
                    '賣出/觀望': float(probabilities[i, 0]),
                    '持有': float(probabilities[i, 1]),
                    '買入': float(probabilities[i, 2])
                },
                ...
                'top_features': [
                    {
                        ...
                        'impact': '推動決策' if info['sensitivity'] > 0 else '抑制決策'
                    }
...
class StockTradingModel:
    ...
    def train(self, X_train, y_train, config=None):
        ...
        print(f"使用配置：embedding_dim={config.EMBEDDING_DIM}, hidden={config.HIDDEN_DIMS}, "
    ...
    def evaluate(self, X_test, y_test, enable_explanation=True):
        ...
        if enable_explanation and len(X_test) > 0:
            ...
            if len(buy_signals) > 0:
                examples.append(('買入', buy_signals[-1]))
            if len(sell_signals) > 0:
                examples.append(('賣出', sell_signals[-1]))
            if len(hold_signals) > 0:
                examples.append(('持有', hold_signals[-1]))
            
            for signal_type, idx in examples[:3]:
                print(f"\n▶ {signal_type}信號 - 日期: {X_test.index[idx].date() if hasattr(X_test, 'index') else f'第{idx}筆'}")
                ...
                print(f"  預測: {explanation['prediction_label']}")
                print(f"  信心度: 買入={explanation['probabilities']['買入']:.1%}, "
                      f"持有={explanation['probabilities']['持有']:.1%}, "
                      f"賣出={explanation['probabilities']['賣出/觀望']:.1%}")
                
                print(f"\n  關鍵影響因素（前5名）：")
                for i, feat in enumerate(explanation['top_features'][:5], 1):
                    print(f"    {i}. {feat['name']:<20} = {feat['value']:>8.4f}  "
                          f"(敏感度: {feat['sensitivity']:>+8.4f}) - {feat['impact']}")
...
def daily_inference(stock_id, model_path, enable_explanation=True):
    """
    每日收盤後執行的推論腳本（增強版：含決策解釋）
    
    Args:
        stock_id: 股票代碼
        model_path: 模型路徑
        enable_explanation: 是否輸出決策解釋
    ...
    print(f"\n{action_emoji[prediction]} 預測結果：{action_map[prediction]} (建議操作)")
    print(f"   買入機率：{probabilities[2]:>6.1%} {'█' * int(probabilities[2]*20)}")
    print(f"   持有機率：{probabilities[1]:>6.1%} {'█' * int(probabilities[1]*20)}")
    print(f"   賣出機率：{probabilities[0]:>6.1%} {'█' * int(probabilities[0]*20)}")
    
    # 7. 決策解釋
    explanation = None
    if enable_explanation:
        print(f"\n{'─'*70}")
        print("🔍 決策分析：為什麼模型做出這個預測？")
        print(f"{'─'*70}")
        ...
        for i, feat in enumerate(explanation['top_features'], 1):
            impact_symbol = '↑' if feat['sensitivity'] > 0 else '↓'
            print(f"   {i}. {feat['name']:<20} = {feat['value']:>8.4f}")
            print(f"      敏感度: {feat['sensitivity']:>+8.4f} {impact_symbol} {feat['impact']}")
    
    # 8. 顯示關鍵技術指標
    print(f"\n{'─'*70}")
    print("📈 技術指標總覽：")
    print(f"{'─'*70}")
    print(f"   RSI (相對強弱)：{df.iloc[-1]['RSI']*100:.2f} "
          f"{'🔥超買' if df.iloc[-1]['RSI'] > 0.7 else '❄️超賣' if df.iloc[-1]['RSI'] < 0.3 else '✅中性'}")
    print(f"   SMA乖離率：{df.iloc[-1]['SMA_Deviation']:.2%} "
          f"{'⬆️偏高' if df.iloc[-1]['SMA_Deviation'] > 0.05 else '⬇️偏低' if df.iloc[-1]['SMA_Deviation'] < -0.05 else '➡️正常'}")
    print(f"   ATR標準化（波動）：{df.iloc[-1]['ATR_Normalized']:.4f}")
    print(f"   5日對數收益：{df.iloc[-1]['LogReturn_5']:.4f}")
    print(f"   10日對數收益：{df.iloc[-1]['LogReturn_10']:.4f}")
    print(f"   20日對數收益：{df.iloc[-1]['LogReturn_20']:.4f}")
...
def main():
    ...
    print("="*80)
    print("🚀 AI 金融交易系統 - 台股 MVP 專案 (投資組合模式 + 決策解釋)")
    print("="*80)
    ...
    print("\n📊 當前系統配置：")
    print("-"*80)
    print(f"模型架構：Embedding={config.EMBEDDING_DIM}, Hidden={config.HIDDEN_DIMS}, Dropout={config.DROPOUT}")
    print(f"訓練設定：LR={config.LEARNING_RATE}, Epochs={config.EPOCHS}, Episodes={config.EPISODES_PER_EPOCH}")
    print(f"交易策略：買入門檻={config.BUY_THRESHOLD:.2%}, 賣出門檻={config.SELL_THRESHOLD:.2%}")
    print(f"Few-Shot：K={config.K_SHOT}, Q={config.Q_QUERY}, Temperature={config.TEMPERATURE}")
    print("-"*80)
    print("💡 如需調整參數以優化模型，請參閱 '實驗調整指南.md'\n")
